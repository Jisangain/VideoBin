# One ip will be counted as one view per 12 minutes
import datetime
...
def is_a_new_viewer(ip_address):
    ...
    if not get_ip:
        ...
        if get_ip.time < datetime.datetime.now() - datetime.timedelta(minutes=1):
...
def distribute(old_earnings, current_earnings):
    try:
